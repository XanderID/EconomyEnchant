...
class GUI
{
        ...
        if($invmenus !== null) {
            ...
            $invmenu->setListener(function (InvMenuTransaction $transaction) use ($page, $invmenu): InvMenuTransactionResult {
                ...
                if($name == ("§fNext To Page: " . ($this->remainingPage + 1))) {
                    ...
                    return $transaction->discard();
                } elseif($name == ("§fPage: " . ($this->remainingPage + 1))) {
                    return $transaction->discard();
                }
    ...
    private function submit(Player $player, Item $itemBook, InvMenu $invmenu, int $page)
    {
        ...
        $invmenu->setListener(function (InvMenuTransaction $transaction) use ($display, $price, $nextlevel, $enchant, $paper): InvMenuTransactionResult {
            $player = $transaction->getPlayer();
            $item = $transaction->getItemClicked();
    ...
    public function createPage(Item $item): void
    {
        ...
        foreach($splited as $index => $splist) {
            foreach($splist as $encdata) {
                $itemBook = StringToItemParser::getInstance()->parse("enchanted_book");
                ...
                if($encdata["enchant"]->getMaxLevel() == $nowlevel) {
                    continue;
                }
                ...
                $itemBook->setLore(str_replace("{price}", "" . ($encdata["price"] * $nowlevel), $this->gui["buy-shop"]["lore"]));
